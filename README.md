...
The defining design choice: **a dataset type is defined entirely in YAML** — fields, validation rules, generation prompts, judge criteria, and export templates. Adding a new domain takes zero Python. The repo ships two (`support_tickets`, `product_reviews`) to prove it.
...
- **Tested** — 38 pytest tests, including the LLM stages exercised offline with mocked calls.
...
│   ├── .env.example
│   └── domains/
│       ├── support_tickets.yaml  # full domain spec
